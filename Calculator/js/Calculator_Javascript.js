...
const Calculator = {
    ...
    Display_Value: '0',
    //This will hold the firs operand for any expressions, we set it to null for now
    First_Operand: null,
    //This checks wether or not the second operand has been iputted by the user
    Wait_Second_Operand: false,
    //This will hold the operator, we set it to null for now
    operator: null,
};

//This modifies values each time a button is click on
...
function Input_Decimal(dot) {
    //This ensures that accidental clicking of the decimal pont doesn't
...
function Handle_Operator(Next_Operator) {
    ...
    //then check updates the operator an exits from the funtion
...
keys.addEventListener('click', (event) => {
    //The target variable is an object that presents the element
    //that was clicked
    const { target } = event;
    //If the element that was clicked on its not a button, exit the function
